    ...
    <table class="bxb-sp-settings-table w-full border border-slate-100 mt-5 text-sm text-left text-gray-500 rounded-sm">
        <thead class="text-xs text-gray-900 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <th scope="col" class="bxb-sp-settings-table-days-th px-6 py-3 border border-slate-200">Post Name</th>
                <th scope="col" class="bxb-sp-settings-table-days-th px-6 py-3 border border-slate-200">New Post Date</th>
                <th scope="col" class="bxb-sp-settings-table-days-th px-6 py-3 border border-slate-200">Author</th>
            </tr>
        </thead>
        <tbody>
            <?php
                foreach($records as $record) {
                    ...
                    <?php 
                            if(empty($post_title)){
                        ?>
                            <td class="px-6 py-4 border border-slate-200 text-gray-900">Title can't be fetched. This post might have been deleted.</td>
                        <?php 
                
                            } else {
                        ?>
                            <td class="px-6 py-4 border border-slate-200 text-gray-900"> <?php echo '<a href="'.$post_link.'">'.$post_title.'</a>'; ?></td>
                        <?php 
                            }
                        ?>
                            <td class="px-6 py-4 border border-slate-200 text-gray-900"> <?php echo $record->new_post_date; ?></td>
                            <td class="px-6 py-4 border border-slate-200 text-gray-900"> <?php echo $display_name; ?></td>
                        </tr>
                    <?php
                }
            ?>
