<?php
    if(isset($_POST['old_string'])) {
        ...
        if($flag){
            echo '<div class="p-4 mt-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50" role="alert">
                    <span class="font-medium">Success!</span> The string has been replaced in the database.
                </div>';
        }else{
            echo '<div class="p-4 mt-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50" role="alert">
...
<form action="<?=$form_action?>" method="post" class="max-w-sm mx-left mt-5 ml-5 mb-5">
    ...
    <div class="mb-5">
        <label for="new_string" class="block mb-2 text-sm font-medium text-gray-900">New String (#)</label>
